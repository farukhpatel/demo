...
const UpdateShopAddress = (props) => {
    ...
    useEffect(() => {
        instance.get(`${API.GET_LOCALITIES_BY_CITY_ID}&city_id=${addressForm.city_id}`).then(res => {
    ...
    const UpdateShopAddressFormSubmit = (e) => {
        ...
        instance.patch(`${API.UPDATE_SHOP_ADDRESS}/${id}`, addressForm).then(function (response) {
                ...
                <div className="vendor-form-1 address-form">
                    ...
                    <form className="vendor-form">
                        ...
                        <div class="form-group">
                            ...
                            {cities && cities.length > 0 ? (
                                <select onChange={(event) => handleCitySelect(event)}>
                                    ...
                                    {cities.map((city) => {
                                        return <option value={city?.id} selected={addressForm.city_id === city?.id ? 'selected' : ''} label={city?.city} />;
                                    })}
                                </select>
                            ) : (
                                    "No Cities Found."
                                    ...
                                    {addressForm &&
                                        ...
                                        localities.map((locality) => {
                                            return (
                                                <option
                                                    value={locality?.id}
                                                    selected={addressForm?.locality_id === locality?.id ? 'selected' : ''}
                        ...
                        <button
                            ...
                            onClick={(event) => { UpdateShopAddressFormSubmit(event) }}
                        >
                            Submit
                </button>
                    </form>
