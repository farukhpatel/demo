...
function PaymentSettlement() {
  ...
  const Submits = (e) => {
    ...
    // GET_ORDER_SALES
    let url =
      `${API.GET_ORDER_SALES}start_date=${start_date}&end_date=${end_date}&get_transaction=${get_transaction}` +
      (id > 0 ? "&shop_id=" + id : "");
    // console.log(url)
  ...
  const CheckboxHandle = async (order, index) => {
    if (checked[index]) {
      ...
      let transaction_commission = parseFloat(
        (unpaid2.transaction_commission - order.order_commission).toFixed(2)
      );
      let transaction_amount = parseFloat(
        (
          unpaid2.transaction_amount -
          (order.order_net_amount - order.delivery_charge)
        ).toFixed(2)
      );
      let transaction_payable_amount = parseFloat(
        (unpaid2.transaction_payable_amount - order.payable_amount).toFixed(2)
      );
      let transaction_tax = parseFloat(
        (unpaid2.transaction_tax - order.order_tax).toFixed(2)
      );
      let obj = {
      ...
      let paymentfilter = paymentSettlementFilter;
      setPaymentSettlementFilter(
        paymentfilter.filter((value, i) => {
          ...
          <div className="main-root-second">
            {toggleIndex === 0 && unpaid?.length > 0 ? (
              ...
              >
                {modalOpen && (
                  <SettleModal
                    paymentSettlementFilter={paymentSettlementFilter}
                    unpaid={unpaid2}
                    start_date={from}
                    end_date={to}
                    id={id}
          ...
          <div className="myorders-inner-div paymentsettle-inner-div">
            ...
            <div class="tab-content" id="myTabContent">
              <div
                ...
                role="tabpanel"
                aria-labelledby="remaining-payments"
              >
                        ...
                        return (
                          <tr>
                            <th scope="row">
                              <input
                                type="checkbox"
                                onClick={(e) => CheckboxHandle(order, index)}
                      ...
                      paid.map((value, index) => {
                        return (
                          <tr>
                            ...
                            <td>
                              {moment(value.transaction_date).format(
                                "YYYY-MM-DD"
                              )}
                            </td>
